...
// 遗留问题：动态代理更加灵活，https://blog.csdn.net/m0_38138387/article/details/79094565 交易中介类比。
    ...
    /**
     * 1.删除一个字符串中出现次数最多的字符，如果多个字符出现次数一样，则都删除。要求字符顺序不变。
     * 2.一列数的规则如下: 1、1、2、3、5、8、13、21、34...... 求第30位数是多少， 用递归算法实现。
     * 3.N个1到1000之间的随机整数（N≤1000），对于其中重复的数字，只保留一个，把其余相同的数去掉。
     * 然后再把这些数从小到大排序。
     */

    public static List<Integer> sortSth(Integer N) {
        //构造 N个1到1000之间的随机整数（N≤1000）
        List<Integer> numbers = new ArrayList<>();
        for (int i = 0; i < N; i++) {
            numbers.add(1+(int)(Math.random()*1000));
        }
